  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email, source: "insurance_checker" }),
      });
    ...
    <div className="flex flex-col items-center gap-6 text-center">
      ...
      <div>
        <h2 className="text-2xl font-bold text-brand-text-primary font-heading">
          Get your coverage report
        </h2>
        <p className="text-brand-text-secondary mt-2 text-sm">
          Enter your email to see your personalized insurance coverage estimate.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3">
        ...
        >
          {status === "loading" ? "Loading…" : "See My Coverage"}
        </button>
      </form>
